...
import { TypedDataUtils, EIP712TypedData } from "eth-sig-util";
...
export function getDataString(func: string, arrVals: string[]): string {
	let val = "";
...
export function encodeTypedDataMessage(msg: string): string {
	const data = TypedDataUtils.sanitizeData(JSON.parse(msg) as EIP712TypedData);
...
export async function verifySignature(address: string, sig: string, hash: string, chainId: number): Promise<boolean> {
